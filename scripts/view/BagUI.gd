...
const TOKEN_DESCRIPTIONS := {
	...
	"F": "朝当前朝向前进一格。",
	"B": "朝当前朝向后退一格。",
	"SL": "朝当前朝向的左侧横移一格，不改变面朝方向。",
	"SR": "朝当前朝向的右侧横移一格，不改变面朝方向。",
	"DS": "朝当前朝向快速冲刺两格；遇到阻挡时会提前停下。",
	"HK": "向前两格内甩出钩拽，命中最近目标后将其拉近一格。",
	"SB": "朝面前发动盾击，造成伤害并将目标击退一格。",
	"HM": "朝面前砸出一个前方 2x3 的锤击范围。",
	"RA": "挥出围绕自身的旋转斧，覆盖周围 3x3 区域中的其他格子。",
	"PI": "朝当前朝向发动前方 1x4 的直线穿刺。",
	"TH": "朝当前朝向发动两格贯刺，攻击直线上的目标。",
	"SW": "朝当前朝向发动横扫，覆盖前方与左右三格。",
	"BW": "朝面前射出一箭，命中射线内最近的敌人；会被墙体阻挡。",
	"CA": "发动十字刃攻击，同时覆盖上下左右四个相邻格。",
	"TL": "原地向左转，改变后续动作朝向。",
	"TR": "原地向右转，改变后续动作朝向。",
	"A": "发动基础斩击，攻击面朝方向上的目标。",
